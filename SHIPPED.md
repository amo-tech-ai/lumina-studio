...
### Added
- **Onboarding is now its own guided flow.** Thirteen screens with predictable
  back/next navigation, instead of a route that behaved differently depending on
  how you arrived.
...
- **Locked-down tables re-secured.** Two sets of internal tables had quietly
  regained read access for signed-in users. Both are locked again, and automated
  tests now catch it. *(We're still hunting why the settings drifted — it's
  happened twice.)*
