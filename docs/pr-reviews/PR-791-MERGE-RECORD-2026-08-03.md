...
| Inert Sign-out control failure path uses `signoutCandidatesProbed` | ✅ implemented |
| `13f_signout_idempotent`: `redirect: "manual"` + 3xx to `/login` | ✅ |
| `evidence_out` repo-relative; `blob:` URLs sanitized | ✅ |
| Overall preview journey | `overall_pass: true`, `hard_ac_pass: true`, `soft_perf_pass: false` |
...
- **Sibling PR #764** merged at `b12e4a5d39d70c7b15f55178ddf31102aaa5cb0e` (~53 minutes before this evidence PR). Evidence proves click-triggered sign-out works but does not cryptographically tie the deployed Worker to #764's commit.
