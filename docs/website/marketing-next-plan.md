...
|---|---|---|---|
| 1 | `/` | `app/src/app/(web)/page.tsx` | **New** |
| 2 | `/services/fashion-photography` | `app/src/app/(web)/services/fashion-photography/page.tsx` | **New** |
| 3 | `/services/ecommerce-photography` | `app/src/app/(web)/services/ecommerce-photography/page.tsx` | **New** |
| 4 | `/services/clothing` | `app/src/app/(web)/services/clothing/page.tsx` | **New** |
| 5 | `/services/amazon` | `app/src/app/(web)/services/amazon/page.tsx` | **New** |
| 6 | `/services/location` | `app/src/app/(web)/services/location/page.tsx` | **New** |
| 7 | `/services/jewellery` | `app/src/app/(web)/services/jewellery/page.tsx` | **New** |
| 8 | `/services/instagram` | `app/src/app/(web)/services/instagram/page.tsx` | **New** |
| 9 | `/services/video` | `app/src/app/(web)/services/video/page.tsx` | **New** |
| 10 | `/services/shopify` | `app/src/app/(web)/services/shopify/page.tsx` | **New** |
| 11 | `/login` | `app/src/app/(web)/login/page.tsx` | **New** |
...
```
app/src/app/
  layout.tsx                      ← stripped: html/body/fonts only
  (web)/
    layout.tsx                    ← NEW: Header + Footer, no CopilotKit
    page.tsx                      ← Home page
    ...
    login/page.tsx
  (app)/
    layout.tsx                    ← NEW: CopilotKit + OperatorPanel (moved from root)
    page.tsx                      ← Command Center (existing, moved)
    brand/page.tsx                ← existing (unchanged)
    shoots/page.tsx               ← existing (unchanged)
    assets/page.tsx               ← existing (unchanged)
    campaigns/page.tsx            ← existing (unchanged)
    matching/page.tsx             ← existing (unchanged)
  not-found.tsx                   ← shared 404
```

The marketing layout is a simple server component that provides `<MarketingHeader>` and `<MarketingFooter>` around `{children}`, with NO `<CopilotKit>` provider. The operator layout is moved to `(app)`.

---

## 3. Shared Marketing Layout Plan

### `(web)/layout.tsx` (Server Component)

```tsx
import { MarketingHeader } from "@/components/web/header";
import { MarketingFooter } from "@/components/web/footer";
...
### Route group safety validation

Before porting any pages, confirm the layout isolation works:

- `(web)` layout = `<MarketingHeader>` + `<MarketingFooter>` + `{children}` — no CopilotKit, no OperatorPanel
- `(app)` layout = `<CopilotKit>` + `<OperatorPanel>` — operator routes only
- Root `layout.tsx` = just `<html>`/`<body>` with fonts/suppressHydrationWarning
- No operator UI leaks into web pages

### Why route groups

- Marketing and operator both need `/` — can't have two page.tsx in the same segment
- Route groups let them coexist with different layouts
- Next.js tree-shakes CopilotKit bundles from web pages
- No client-side pathname checks needed

---

## 4. Shared Components Needed

All new components live in `app/src/components/web/` to keep them distinct from both Vite imports and operator components.

### Layout (server components)
| Component | File | Description |
|---|---|---|
| `MarketingHeader` | `components/web/header.tsx` | Port of Vite `Header.tsx`. Replace `<a>` with `<Link>`, remove React Router deps. Scroll-aware via CSS or minimal `use client`. |
| `MarketingFooter` | `components/web/footer.tsx` | Port of Vite `Footer.tsx`. All hrefs mapped to Next.js routes. |

### Homepage sections (server components — no client JS)
| Component | File | Description |
|---|---|---|
| `HeroSection` | `components/web/hero-section.tsx` | Port of Vite component. Replace `<img>` with `next/image`. |
| `ServicesSection` | `components/web/services-section.tsx` | Port of Vite component. 8-card grid. |
| `PortfolioSection` | `components/web/portfolio-section.tsx` | Port of Vite component. 6-image grid. |
| `ProcessSection` | `components/web/process-section.tsx` | Port of Vite component. 4-step grid. |
| `ClientsSection` | `components/web/clients-section.tsx` | Port of Vite component. Text brand names. |
| `CTASection` | `components/web/cta-section.tsx` | Port of Vite component. Static contact form. |

### Service page sub-components (client components — interactive)
| Component | File | Description | Interactive |
|---|---|---|---|---|
| `FashionPackages` | `components/web/fashion-packages.tsx` | Pricing toggle + cards | Yes — useState, useInView |
| `ClothingSlider` | `components/web/clothing-slider.tsx` | Horizontal scroll slider | Yes — useState, scroll |
| `EcommerceExtension` | `components/web/ecommerce-extension.tsx` | 7-section addon panel | Yes — useState, slider |
| `FAQ` | `components/web/faq.tsx` | Reusable accordion FAQ | Yes — shadcn Accordion |
| `AnimatedSection` | `components/web/animated-section.tsx` | Scroll-triggered fade wrapper | Yes — IntersectionObserver |

### Image migration note

Copy only images used by the 12 marketing pages (48 files). Keep unused assets (e.g., dashboard-only images, operator-only images) out of `app/public/images/`. Vite assets remain untouched in `src/assets/`.
...
**Approach:** Load both font families via `next/font/google` in the `(web)` layout only. The operator `(app)` layout keeps Geist.

```tsx
// app/src/app/(web)/layout.tsx
...
All web components are scoped under `.web` so these tokens never leak into the operator UI.
...
- Fallback to `og/home.jpg` via `metadataBase` in `(web)` layout
...
4. Visual appearance matches Vite dev server (`:8080`) pixel comparison on:
...
7. `npm run build` succeeds (no type errors)
8. `npx tsc --noEmit` passes
9. All `<a>` replaced with `<Link>` for internal navigation
10. All `<img>` replaced with `next/image` (or `<img>` with justification)
11. All `VITE_*` env vars replaced with `NEXT_PUBLIC_*`
12. No Vite imports or React Router APIs in `app/src/`
13. No new Supabase, operator, or auth dependencies imported by marketing pages
14. Vite dev server still works unchanged
