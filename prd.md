...
**Date:** 2026-07-09 · **Verified:** 2026-07-18 (Planner / Cloudflare gateway / Campaigns / migration count / citations)  
**Audience:** Engineers and AI agents building iPix  
**Grounded in:** `tasks/plan/audit/00-05` (forensic reconciliation of all prior planning), `tasks/cloudflare/plan/cf-000-platform-architecture.md` (Approved), `tasks/cloudflare/plan/ai-agent-architecture.md` (Approved), `tasks/cloudflare/CLOUDFLARE-EPIC.md`/`todo.md`, `tasks/cloudflare/mastra/MASTRA-EPIC.md`, `linear/issues/IPI-476` through `IPI-483`, Planner design prompts under `Universal-design-prompt-4/` (SCR-32/33/34)  
**Cross-lane NOW:** [`todo.md`](./todo.md) · Planner/Supabase queue: [`tasks/prime/todo`](./tasks/prime/todo)
...
**Current state in one paragraph.** *(Verified 2026-07-18.)* The core operator product is real and working: Brand, CRM, Booking, and Shoot are shipped features with real Mastra agents, real Supabase schemas, and real UI. Vercel still hosts production Next.js; OpenNext/`wrangler` exist for a Workers cutover that has **not** completed. AI Gateway direction shifted 2026-07-14: custom `services/cloudflare-worker/` is **frozen**; native dashboard gateway `ipix-prod` + Workers AI binding (**IPI-586**) is the path forward — still **zero** production traffic through `ipix-prod`. Planner **data boundary is largely shipped** (schema, engine, mutation RPCs, `planner_create_instance` + phase-completeness enforcement — IPI-649 / IPI-653 / IPI-670 on `main`); Hub/Workspace UI shells exist but are incomplete (create-from-Hub **IPI-650** next; workspace views still thin). Three areas remain thin relative to the rest of the product: **Campaign** (schema + `SectionPlaceholder` only), **Planner Hub create + rich workspace views**, and **Intelligence** (side panel only — no standalone page; product decision still open).
...
**Non-goals (explicitly out of scope for the current build horizon):** native mobile apps, two-way external calendar sync (Google/Outlook — read-only ICS export only), full resource-leveling/PERT optimization, a general-purpose agent platform on Cloudflare Agents SDK (evaluated, not adopted). **Payments:** Mercur owns marketplace commerce catalog; Stripe is **in-repo** as the payments backbone (see `docs/stripe/` / commerce proofs) — not “no Stripe planned.” Operator SaaS subscription billing remains a separate P2 track.
...
Cloudflare Workers (via OpenNext) ── migration in progress, see §4
 ...
 └──────────────────────┬───────────────────────────────────────┘
                        │
 ┌──────────────────────▼─────────────────────────────────────────┐
 │ AI path (2026-07-14) — native AI Gateway `ipix-prod` + Workers │
 │ AI binding (IPI-586+). Custom `services/cloudflare-worker/` is │
 │ frozen/contained. Agents still mostly call Gemini directly     │
 │ until the native path is proven.                               │
 └──────────────────────┬─────────────────────────────────────────┘
                        │
 ┌──────────────────────▼─────────────────────────────────────────┐
 │ Supabase — Postgres (system of record), Auth, pgvector,        │
 │ Realtime (notifications; planner sync still mostly poll/REST)  │
 └──────────────────────────────────────────────────────────────────┘
                        │
 ┌──────────────────────▼─────────────────────────────────────────┐
...
**Hosting direction:** `cf-000-platform-architecture.md` §6 once said Vercel hosts Next.js permanently. Later direction targets OpenNext on Cloudflare Workers (`app/wrangler.jsonc`, `app/open-next.config.ts`). **As of 2026-07-18:** Vercel is still the production host; OpenNext cutover gates are incomplete. Treat Workers hosting as the **target**, not the live production default.

**AI Gateway direction (verified 2026-07-18):** stop investing in the custom `services/cloudflare-worker/` path (frozen after public-route containment). Prove traffic through managed gateway `ipix-prod` + `workers-ai-provider` (**IPI-586**), then migrate agents (**IPI-594**). Live status: [`tasks/cloudflare/todo.md`](./tasks/cloudflare/todo.md).
...
*(Corrected 2026-07-09 — this section previously claimed a third enforcement level, "service-role edge functions are the only code path that performs the actual write, never a Mastra tool directly." Verification against `app/src/mastra/tools/booking-tools.ts` found Mastra tools call Supabase RPCs via a user-scoped client, with RLS — not a service-role edge function — as the enforcement boundary. The HITL gate is real at UI/workflow levels; only the data-layer mechanism was mis-described. Architecture diagrams, when present, live under `docs/architecture/diagrams/` — some numbered filenames from earlier drafts are not on `main`.)*
...
| **Hyperdrive** | 🔬 Later (CF-DB track) | Not wired today. Future Worker SQL only via Hyperdrive → **direct** Postgres — never Hyperdrive + Supavisor together. See `tasks/prime/supabase-plan.md`. |
...
**What stays here, because it's stable:** the hosting migration is Vercel → Cloudflare Workers via OpenNext (in progress; Vercel still prod). The AI Gateway track is **native `ipix-prod` first** (prove one call via **IPI-586**), not further custom-Worker features. Custom Worker remains frozen until deletion is gated (**IPI-592**). Live task status: `tasks/cloudflare/todo.md`.

### 4.4 Provider strategy

| Tier | Target MVP provider | Fallback |
|:---:|:---:|:---:|
| default | Workers AI (via `ipix-prod`) | Gemini |
| fast | Workers AI (via `ipix-prod`) | Gemini |
| structured | Workers AI (via `ipix-prod`) | Gemini |
| vision | Gemini | NVIDIA NIM (eval) |
| embedding | Workers AI | Gemini (text-embedding) |

**Key rule (target):** no agent imports a provider SDK directly; inference goes through a provider adapter → AI Gateway. **Current reality (2026-07-18):** Mastra agents still resolve Gemini (and related paths) directly; native gateway has **zero** app traffic. Custom Worker path is frozen — do not plan new work against `services/cloudflare-worker/` (see canceled IPI-454 / IPI-461 / sibling issues). Next proof: **IPI-586**.
...
1. **Provider abstraction** — every agent calls inference through a shared provider adapter → AI Gateway, never a vendor SDK directly (not yet enforced — see §4.4; native path starts at **IPI-586**).
...
**Current state:** *(Re-verified 2026-07-18 — unchanged.)* Route `app/(operator)/app/campaigns/page.tsx` is still a `SectionPlaceholder` (title “Campaigns”, issue pointer `IPI2-119`). Database schema deployed (`public.campaigns`, migration `20260707100000_ipi268_campaigns_schema.sql`). **No dedicated lib module, no `/api/campaigns` route, no Campaign agent.** `creative-director` Mastra agent exists but has zero tools and is not the Campaign Agent described in the architecture doc. Next product ticket: **IPI-249** (Campaigns workspace).
...
### 6.7 Planner — 🟡 Data boundary shipped; Hub create + rich workspace still open

**Current state:** *(Verified 2026-07-18.)*

| Layer | Status | Evidence |
|---|:---:|---|
| Lib / engine | 🟢 | `app/src/lib/planner/` — `engine.ts`, `mutations.ts`, `queries.ts`, permissions, tests |
| Schema / RPCs | 🟢 | `planner.*` on remote; mutation RPCs (**IPI-649**); `planner_create_instance` + app adapter (**IPI-653**); phase-completeness enforcement (**IPI-670** / PR #430) |
| Routes (shell) | 🟡 | `/app/planner`, `/app/planner/dashboard` (**IPI-576**), `/app/planner/[instanceId]`, `/settings` — Hub/Workspace shells exist; create-from-Hub UI (**IPI-650**) not shipped; workspace view content still thin vs SCR-32 |
| Linked CI gates | 🟡 | **IPI-665** PR #431 open (drift / lint / types) |

Original epic issues `IPI-476`–`IPI-483` remain the deep acceptance/wiring specs for realtime, notifications, and agent tools. **Do not re-open the “zero UI / 2 open PRs” Jul 9 claim** — that is obsolete.

**Target routes (still the product bar):** `/app/planner` (hub + **New Plan**), `/app/planner/[instanceId]` (workspace per SCR-32), `/app/planner/dashboard` (SCR-33), `/app/planner/[instanceId]/settings` (SCR-34 — Members tab MVP; other tabs post-MVP). Design prompts: `Universal-design-prompt-4/` (SCR-32/33/34).

**Data model:** `planner` schema — 10 tables, 3 enums, four-tier RLS — on remote; create path rejects empty / incomplete / duplicate / zero-phase task payloads.
...
**~183** migration files under `supabase/migrations/` as of 2026-07-18 (was 157 on 2026-07-09). Key schemas:
...
| `planner.*` (10 tables: workflows, phases, instances, tasks, dependencies, assignments, events, view_configs, notification_rules, gate_conditions) | Reusable multi-stakeholder scheduling engine | 🟢 Schema + create/mutate RPCs on `main` — UI create Hub still open · see §6.7 |
...
**Design system compliance:** any new UI must match v3 "Zeely Editorial" tokens ([`Universal-design-prompt-4/DESIGN.md`](./Universal-design-prompt-4/DESIGN.md), [`design.md`](./design.md), `app/src/styles/tokens.css`) — pure white/grey/black, Inter, Geist Mono for all numeric data, black primary actions, no orange/beige. Every card-shaped surface extends the shared `Card` primitive; every status indicator uses `StatusChip`; every loading state uses `SkeletonLoader`, never a spinner. *(Marketing pages may still use Cormorant/Outfit per CLAUDE.md — that is a separate surface from the operator shell.)*
...
|---|:---:|---|
| Design-prompt folder churn (`-new` / `-4` / untracked copies) | 🟡 | Prefer **`Universal-design-prompt-4/`** as the live design tree; reconcile stray `-new` copies before citing paths. |
| `config/groq-models.json` deletion risk | 🟢 | **Cleared 2026-07-18** — file present on disk. |
| Custom Worker vs native `ipix-prod` doc drift | 🟡 | **Decision 2026-07-14** — custom Worker frozen; native path is SSOT in `tasks/cloudflare/todo.md`. Keep this PRD §3/§4 aligned. |
| Two Linear issues (IPI-471, IPI-461) cite missing proof files | 🟡 | IPI-461 canceled with custom-Worker freeze; still clean issue text where needed. |
| 5 `CF-MIG-*` hosting tasks have no Linear issue | 🟡 | Open, cheap fix (create under `IPI-487`). |
| `deep-architecture-review.md` Draft vs `cf-000` | 🟡 | Open — action or mark not adopted. |
| Campaign has zero backend beyond its schema | 🔴 | Unchanged — §6.6 / **IPI-249**. |
| Planner Hub create + rich workspace views incomplete | 🟡 | Data boundary shipped (IPI-649/653/670); **IPI-650** Hub create next; SCR-32 views still thin. |
| Native AI Gateway has zero app traffic | 🔴 | **IPI-586** not started — blocks agent migration / Worker deletion. |
| Intelligence standalone-page need is genuinely undecided | ⚪ | Needs a product decision — see §6.8. |
...
| Planner (backend) | 🟢 Core data boundary shipped | §6.7 — create/mutate RPCs on `main` (IPI-649/653/670) |
| Planner (UI) | 🟡 Shells partial | §6.7 — routes exist; Hub create (IPI-650) + SCR-32 views open |
| Campaign | 🔴 Core, not started | §6.6 — schema + placeholder only |
| Intelligence | 🟡 Undecided | §6.8 — product decision pending |
| Cloudflare hosting migration | 🟡 Infrastructure, in progress | §4.3 — Vercel still prod; OpenNext target |
| AI Gateway (native `ipix-prod`) | 🔴 Zero traffic | §4.4 — IPI-586 next; custom Worker frozen |
...
| **AI Gateway** | Managed Cloudflare AI Gateway (`ipix-prod`) + Workers AI binding — the **investment path** as of 2026-07-14. The older custom Worker under `services/cloudflare-worker/` is frozen. See §4.1, §4.3, §4.4. |
