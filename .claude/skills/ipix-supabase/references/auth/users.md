---
name: auth-users
description: Supabase user object schema — id, email, role, app_metadata (provider/providers list), user_metadata (identity data, user-editable — never use in RLS), identities[], is_anonymous. Permanent vs anonymous users. Use when writing RLS policies, profile sync, or debugging auth.uid() vs user_metadata.
topics: [auth, users, user-object, rls, anonymous-users, app-metadata]
applies_to: [src/contexts/AuthContext.tsx, src/services/profileService.ts, supabase/migrations]
source: https://supabase.com/docs/guides/auth/managing-user-data
ipix_warning: user_metadata is user-editable — NEVER authorize from it in RLS. Use auth.uid() and app_metadata only.
---
...
Caution: Just like permanent users, anonymous users use the **authenticated** role for database access.
