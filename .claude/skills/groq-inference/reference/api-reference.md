...
description: Full Groq REST API — chat completions, Responses API, audio, models, batches, files. ~2000 lines; grep or search by endpoint name. Offline mirror.
load_when: request body parameter names, endpoint paths, response fields, citation_options, compound_custom, disable_tool_validation
tags: groq, api-reference, endpoints, chat-completions, batches
source: https://console.groq.com/docs/api-reference
---

# Groq API Reference

> **Large file.** Search for: `Chat`, `Responses`, `Audio`, `Models`, `Batches`, `Files`. Curated patterns → [`api-advanced.md`](api-advanced.md).
