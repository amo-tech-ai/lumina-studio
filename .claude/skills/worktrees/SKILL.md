...
- [ ] Worktree doc salvage gate passed; worktree **removed** after merge (or documented remove-by date) — [worktrees § post-ship](../worktrees/SKILL.md#post-ship-remove-the-worktree-mandatory)
...
### Pre-delete doc salvage gate (mandatory — P0)

**Incident (Jul 2026):** force-removed worktrees dropped untracked `tasks/llm/`, `docs/llm/`, and `Universal-design-prompt-new/` — **unrecoverable** once the directory was gone. Committed-but-unpushed docs were also lost when branches were deleted later.

Before **`git worktree remove`** on any tree you did not create in the last 5 minutes, run the full gate — not just `pre-delete`:

```bash
WT=/path/to/worktree
git -C "$WT" status -sb
git -C "$WT" ls-files --others --exclude-standard   # every untracked path
npm run worktree:pre-delete                          # blocks unpushed commits
node scripts/worktree-health.mjs --pre-delete        # same + behind-main checks
```

**For each untracked or unstaged doc** (`*.md`, `*.mdc`, under `docs/`, `tasks/`, `.claude/skills/`, design packages):

| Situation | Action |
|-----------|--------|
| Same concern as open PR | Stage + commit + push on that branch |
| Different concern | **Split PR** — new branch/PR (docs-only is fine); never mix with code PR |
| Valuable but out of scope | Ask user OR open `ipi/preserve-worktree-docs`-style preservation PR |
| Scratch / duplicate | Delete explicitly after user confirms |
...
If docs must ship: commit them **before** remove — untracked files are not in git history and cannot be recovered from dangling blobs reliably.

Full playbook → [references/ipix-ops.md#pre-delete-doc-salvage-gate](references/ipix-ops.md#pre-delete-doc-salvage-gate).

### Post-ship: remove the worktree (mandatory)

A merged or abandoned task **must not** leave its worktree on disk. Each tree typically holds **1–3 GB** (`node_modules`, `.next`, caches) and becomes a trap for the next agent (`git worktree list` noise, stale branches, "missing" docs that were never pushed).

**When to remove** — same ship session as PR merge (or immediately after user confirms abandon):

```
[ ] PR merged (or task cancelled) — confirm on GitHub
[ ] Pre-delete doc salvage gate passed (above)
[ ] npm run worktree:pre-delete — green
[ ] git worktree remove <path>          # no --force unless gate + backup done
[ ] git worktree prune
[ ] git branch -d ipi/<task>-<slug>     # only after merge; -D never without user ask
[ ] npm run worktree:audit              # inventory ≤ ~3–4 active trees
```
...
**Agent duty at Phase 5:** [ipix-task-lifecycle](../ipix-task-lifecycle/shipping.md) step 13 is not optional — load this skill and run the salvage gate before reporting Done.
...
- **After every shipped or abandoned task:** run [Pre-delete doc salvage gate](#pre-delete-doc-salvage-gate-mandatory--p0) then [Post-ship: remove the worktree](#post-ship-remove-the-worktree-mandatory). Do not end Phase 5 with a merged task still checked out in a sibling worktree.
...
| **iPix: doc salvage + remove** | [Pre-delete doc salvage gate](#pre-delete-doc-salvage-gate-mandatory--p0) → `git worktree remove` |
...
| Run doc salvage gate before remove | Untracked `tasks/` / `docs/` / design packages are lost forever on `--force` |
...
| Removing a merged worktree without scanning untracked docs | Run [pre-delete doc salvage gate](#pre-delete-doc-salvage-gate-mandatory--p0) first |
