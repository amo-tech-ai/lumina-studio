...
-- transaction. Reachability = gate_type not null + all phase tasks done +
-- is_at_least(coalesce(required_role,'manager')) with viewer always denied.
-- Cycle detection runs only when the proposal changes dependency edges
-- (p_proposed_dependency_edges is not null); date-only approvals skip it
-- but still CAS-check p_expected_dependency_edges against the live graph.
--
-- Edit is client-side proposal regeneration (no RPC). Discard persists
-- status='discarded' without shifting dates.
--
...
create index gate_approvals_instance_id_idx on planner.gate_approvals (instance_id);
...
-- Phase 1 reachability (matches PlannerEngine.checkGate + Linear AC):
-- at least one task for (instance, phase) and every such task is done.
-- Role / org / terminal checks live in the public RPCs.
...
comment on function planner.gate_phase_tasks_done(uuid, uuid) is
  'IPI-483 — true when the phase has tasks and all of them are done for the instance. Does not check actor role.';
...
-- Cycle detection over a jsonb edge array [{fromTaskId, toTaskId, ...}].
...
as $$
  ...
  paths as (
    select
      from_task_id,
      to_task_id,
      array[from_task_id, to_task_id]::uuid[] as path,
      false as cycle
    from edges
    union all
    select
      ...
      e.to_task_id,
      p.path || e.to_task_id,
      e.to_task_id = any(p.path)
    from paths p
    join edges e on e.from_task_id = p.to_task_id
    where not p.cycle
      and array_length(p.path, 1) < 256
  )
  select exists (select 1 from paths where cycle);
$$;

comment on function planner.dependency_edges_have_cycle(jsonb) is
  'IPI-483 — DFS cycle check over finish-to-start edges. Used only when an approve proposal changes dependency edges.';
...
-- ── 4. planner_approve_gate ────────────────────────────────────────────────

create or replace function public.planner_approve_gate(
  p_instance_id uuid,
  p_phase_id uuid,
  p_idempotency_key text,
  p_changed_tasks jsonb,              -- [{taskId, expectedUpdatedAt, newStartDate, newEndDate}] may be []
  p_expected_dependency_edges jsonb,  -- live-edge CAS snapshot (may be [])
  p_proposed_dependency_edges jsonb default null  -- null = date-only (skip cycle); non-null = replace edges + cycle check
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if p_proposed_dependency_edges is not null then
    ...
    -- Only finish_to_start in Phase 1.
    for v_edge in
      select
        (e ->> 'fromTaskId')::uuid as from_task_id,
        (e ->> 'toTaskId')::uuid as to_task_id,
    ...
    select
      ...
      (e ->> 'fromTaskId')::uuid,
      (e ->> 'toTaskId')::uuid,
      'finish_to_start'::planner.dependency_type,
...
begin
  ...
  if not exists (
    ...
    where id = p_phase_id and workflow_id = v_workflow_id
  ) then
    return jsonb_build_object('ok', false, 'code', 'NOT_FOUND');
  end if;
