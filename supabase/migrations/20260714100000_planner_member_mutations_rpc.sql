...
begin
  ...
  -- manager's or the owner's assignment requires owner. Matches the role
  -- hierarchy diagram (owner acts on anyone, manager acts strictly below
  -- itself) — a tighter rule than the live assignments_update_owner RLS
  -- policy (owner-only for ANY row), which remains as a blunter backstop
  -- for any caller that somehow bypasses this RPC.
  if v_target_role in ('owner', 'manager') and not planner.is_at_least(p_instance_id, 'owner') then
    raise exception 'planner_update_role: insufficient_role';
  end if;

  if v_target_role = 'owner' then
    -- ponytail: fixed lock order (all owner rows, id-ordered) before ever
    -- locking the target row — both concurrent callers acquire this same
    -- first lock in the same order, so they can only ever block each other
    -- here, never deadlock. See migration header for the general pattern.
...
begin
  ...
  if v_target_role = 'owner' then
    -- ponytail: same fixed lock-order pattern as planner_update_role — see
    -- migration header and that function's comment for the deadlock rationale.
    perform 1 from planner.assignments
      where instance_id = p_instance_id and role = 'owner'
      order by id
      for update;
