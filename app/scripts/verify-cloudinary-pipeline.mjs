 ...
 *
 * Proves the full production media path on the remote project:
 *   signed upload → Cloudinary asset → webhook → Supabase rows →
 *   DNA trigger → signed delivery URL (200 image/*) → cleanup
 *
 ...
 * matching `publicId`. Exits early on first hit. Returns the joined row or null
 * on timeout. `now` and `sleep` are injected for deterministic testing.
 *
 * @param {object} args
 ...
 * @returns {Promise<{asset: object, cloudinaryAsset: object | null} | null>}
 */
export async function pollForWebhookRow({
  ...
  while (now() < deadline) {
    ...
    if (asset?.id) {
      const { data: cloudinaryAsset } = await supabase
...
async function resolveOperatorCookie(supabaseFactory) {
  const cookie = process.env.CLD105_OPERATOR_COOKIE;
  if (cookie) {
    console.log(`PASS operator-auth — using CLD105_OPERATOR_COOKIE (${cookie.length} chars)`);
    return cookie === "dev" ? "" : cookie;
  }
  ...
  const auth = supabaseFactory(
    required("NEXT_PUBLIC_SUPABASE_URL", process.env.NEXT_PUBLIC_SUPABASE_URL ?? process.env.VITE_SUPABASE_URL),
...
async function main() {
  ...
  const supabaseUrl =
    process.env.NEXT_PUBLIC_SUPABASE_URL ?? process.env.VITE_SUPABASE_URL ?? process.env.NEXT_SUPABASE_URL;
  ...
  try {
    ...
    if (!validated.ok || !upRes.ok) {
      stages.push(stage("cloudinary-upload", false, `HTTP ${upRes.status} ${validated.error ?? upText.slice(0, 200)}`));
    ...
    if (!rows?.asset?.id) {
      stages.push(stage("webhook-received", false, "timed out waiting for assets row"));
    ...
    // 6. dna-status
    const dna = interpretDnaState(rows.asset);
