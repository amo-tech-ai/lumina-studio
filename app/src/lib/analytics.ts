 ...
 * - avgBrandDna: avg(score) brand_scores / avg(assets.dna_score) | brand_scores.created_at / assets.created_at | brand_id (assets_brand_id_idx) | null if no rows
 * - avgAssetMatch: avg(assets.dna_score) | assets.created_at | brand_id | null
...
export type AnalyticsOverviewPayload = {
  ...
  /** Verified or null: avg Brand DNA */
  avgBrandDna: number | null;
  /** Verified or null: avg asset match */
...
// Alias for IPI-297 campaign-performance use (same shape; campaign-level unavailable metrics stay null)
export type CampaignPerformancePayload = AnalyticsOverviewPayload;
