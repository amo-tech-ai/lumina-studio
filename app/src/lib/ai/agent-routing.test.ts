...
describe("IPI-607 agent routing flags", () => {
  ...
  it("aliases default to production-planner env key", () => {
  ...
  it("default and production-planner share the same switch", () => {
    const env = {
      AI_ROUTING_AGENT_PRODUCTION_PLANNER: "native",
    };
    expect(resolveAgentRoutingMode("production-planner", env)).toBe("native");
    expect(resolveAgentRoutingMode("default", env)).toBe("native");
  });
  ...
  it("does not read process.env when an env object is passed", () => {
    ...
    try {
      expect(resolveAgentRoutingMode("crm-assistant", {})).toBe("legacy");
      expect(
