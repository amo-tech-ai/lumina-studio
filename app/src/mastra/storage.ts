...
export function getMastraStorage(): MastraAppStorage {
  ...
  // IPI-803: Check for Workers PG scope first
  if (workersPgScopedStoreGetter) {
    const scopedStore = workersPgScopedStoreGetter();
    if (scopedStore) {
      return scopedStore;
    }
    // If on Workers with PG mode but no ALS context, throw
    if (isCloudflareWorkersRuntime() && !shouldSkipMastraPostgresStorage()) {
      throw new MastraStorageUnavailableError(
        "[mastra] Workers PG mode requires withMastraWorkersPgStorage wrapper (IPI-803). " +
          "Call getMastraStorage() inside withMastraWorkersPgStorage().",
      );
    }
  }
...
/** Workers PG scope hook for IPI-803. */
let workersPgScopedStoreGetter: (() => MastraAppStorage | undefined) | undefined;

export function registerWorkersPgScopedStoreGetter(
  getter: () => MastraAppStorage | undefined,
): void {
...
export function withMastraApplicationName(connectionString: string): string {
  try {
    const url = new URL(connectionString);
    url.searchParams.set("application_name", "ipix-mastra");
    return url.toString();
  } catch {
    // If not a valid URL, append as query param
    const separator = connectionString.includes("?") ? "&" : "?";
    return `${connectionString}${separator}application_name=ipix-mastra`;
  }
}
