...
> **Purpose:** single shared reference for the PRAGENT-001..010 rollout — official source registry,
> current pinned versions, verified CLI commands, evidence/scorecard/rollback templates, and the
> source-authority order. Every PRAGENT linear task should link here instead of duplicating URLs.
...
| Config defaults (canonical settings) | https://github.com/The-PR-Agent/pr-agent/blob/main/pr_agent/settings/configuration.toml |
...
| Bedrock model | `bedrock/qwen.qwen3-coder-next` | – | region `us-east-1`; static keys → OIDC in NAS |
...
# pilot pool of PRs (see PRAGENT-006)
...
# reviewer capability matrix (PRAGENT-007)
...
### 4.2 Upstream verification (PRAGENT-002/010)
...
### 4.3 Controlled defect iteration (PRAGENT-005)

```bash
pip install pr-agent
export OPENAI_KEY=...            # or Bedrock env for qwen3
python -m pr_agent.cli --pr_url=<PR_URL> review     # publish_output=false to iterate w/o posting
```
CLI still needs a real PR URL — create one tiny temporary branch + PR per seeded defect, tune locally, run the GitHub-action once, then close without merging.

### 4.4 Cost/measurement (PRAGENT-008) — proxy only, no custom dashboard first
...
Treat per-PR cost as `estimated` until AWS cost explorer attribution exists (PRAGENT-008).
...
## 7. Linear linkage (PRAGENT-001..010)

| PRAGENT | Issue | Priority | Status | Links this registry |
|---|---|---|---|---|
| 001 Audit | IPI-928 | High (P2) | Todo | ✅ evidence: repo files + runs |
| 002 Verify upstream | IPI-929 | High (P2) | Todo | ✅ configuration.toml + releases |
| 003 Expert pack | IPI-661 | Medium (P3) | Todo | ✅ doc URLs |
| 004 Config + workflow | IPI-659 | High (P2) | Todo | ✅ config keys + install docs |
| 005 Seeded defects | IPI-930 | High (P2) | Todo | ✅ CLI + temp-PR patterns |
| 006 Pilot | IPI-660 | High (P2) | Backlog | ✅ gh CLI + scorecard |
| 007 Consolidate | IPI-931 | Medium (P3) | Backlog | ✅ reviewer sources |
| 008 Measure | IPI-932 | Medium (P3) | Backlog | ✅ cost/API sources |
| 009 OIDC | IPI-522 | Urgent (P1) | Backlog | ✅ AWS docs + exact SHA pinning |
| 010 Upgrade pin | IPI-933 | High (P2) | Backlog | ✅ releases/tags |