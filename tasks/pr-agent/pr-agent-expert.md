...
> **Verdict v2 applied:** reviewer corrections (manual-command allowlist, seeded-defect validation, non-absolute expert rules, docs/config task split, pinned version) merged into §4, §5, §6, §7, §9.
...
- Context loads from the **default branch** (`repo_context_from_default_branch`), which also protects instructions from PR prompt-injection.
...
6. **Manual `/` commands ungated** — anyone who can comment on a PR can trigger Bedrock-backed commands (needs allowlist in PRAGENT-002/004).
...
| `restricted_mode`, `ignore_pr_labels`, `ignore_pr_authors`, `persistent_comment`, `publish_output_no_suggestions`, `num_max_findings`, `extra_instructions` | Verified | same `configuration.toml` |
...
- The **existing pinned Action SHA stays unchanged throughout the pilot** (PR#006/PR#009 owns any version bump later).
...
# custom_model_max_tokens = 250000   # PR-Agent token-cap override; re-validate vs upstream default at implement

# — knowledge (staged, §5) —
repo_context_files = ["AGENTS.md", "docs/pr-review-guidelines.md"]
repo_context_from_default_branch = true   # verify key exists upstream
# repo_context_max_lines = 500             # default; raise ONLY after measured overflow
...
# — persistence —
persistent_comment = false        # one top-level comment, not thousands
# publish_output_no_suggestions = true   # suppress empty-output noise (verify at implement)
```
...
> **Numbering note:** `tasks/pr-agent/pr-agent-plan.md` (approved rollout) labels the same work `PRAGENT-001…006` — its `002` (contract+config), `003` (pilot), `004` (consolidate), `005` (measure), `006` (OIDC) map 1:1 onto the list below. This plan **renumbers** to the reviewer-recommended 9-task sequence so the new `seeded-defect validation` (before pilot) and the `version upgrade` (last) steps get their own IDs. When both docs are read together, prefer the numbering here.
...
```
IPI-TBD · PRAGENT-002.. — Implement iPix PR-Agent expert configuration.
...
6. Context = §5 staged. Start AGENTS.md + pr-review-guidelines.md at 500; measure; then add 4
   specialist sheets; niche sheets on demand only. Do NOT load all sheets at once.
