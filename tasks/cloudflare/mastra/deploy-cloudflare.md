...
> **⛔ iPix SSOT — reference only (Jul 2026)**  
> **Do not use `@mastra/deployer-cloudflare` as the primary path.** iPix runs Mastra **in-process inside OpenNext** (`app/wrangler.jsonc`). This doc is upstream Mastra reference for a **future** standalone Worker split only.  
> **Canonical:** `tasks/cloudflare/mastra/MASTRA-EPIC.md` · [Mastra: deploy via web framework](https://mastra.ai/guides/deployment/cloudflare)
