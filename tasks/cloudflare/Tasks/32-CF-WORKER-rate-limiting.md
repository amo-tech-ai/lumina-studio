---
status: "CANCELLED (2026-07-13) — superseded by 014-CF-GW-enable-rate-limiting.md, which achieves the same goal via a dashboard toggle with zero new code. This task would add new custom Worker-code middleware, which contradicts the migration's entire goal of removing hand-rolled gateway logic. Kept for history, not for execution."
