...
**Status:** 🟡 PARTIALLY STALE (corrected 2026-07-13) — secrets/env-var mechanism is real and working, but the specific `MODEL_REGISTRY_OVERRIDE: workers-ai-only` claim below no longer matches reality
**Verified:** Secrets accessible in Worker, no hardcoding in code

**Correction (2026-07-13):** the live `services/cloudflare-worker/src/model-registry.ts` supports `"gemini" | "workers-ai" | "bedrock" | "nvidia"` as providers, not a `workers-ai-only` override. Whoever picks this task back up should re-verify the actual current `MODEL_REGISTRY_OVERRIDE` value (if the env var still exists at all) rather than trusting the table below.
