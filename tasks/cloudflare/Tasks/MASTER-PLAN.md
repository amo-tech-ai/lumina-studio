...
## Corrections (2026-07-13) — read before using this plan

1. **This table's links are stale.** They point to filenames like `IPI-XXX-CF-AI-020-add-workers-ai-binding.md`, but the actual files on disk use 3-digit prefixes (e.g. `003-CF-AI-add-workers-ai-binding.md`). Match by Task ID, not by clicking the link.
2. **This 10-task table is a summary, not the full plan.** The actual folder has 30+ task docs — this table only lists the high-level phases. See `000-Architecture-Decision.md` for the full picture and the phased rollout order (fix PR #339 → extract PR #342's useful fixes → proof of concept → full test matrix → staging → gradual rollout → delete custom code last).
3. **Tasks `029`-`034` and `054` (deploying Mastra as its own Cloudflare Worker) are Blocked**, not part of the near-term plan — they assume an architecture (Mastra as a separately-deployed Worker) that doesn't match today's reality (Mastra runs inside the Next.js app). Don't start these without a deliberate decision to change that first.
4. **Task `039` (AI Search/RAG for brand PDFs) is speculative scope creep** — no established requirement currently justifies it. Not part of this migration.
5. **See `000-Architecture-Decision.md`** for the authoritative, currently-accurate version of this plan, including which of these 10 summary tasks correspond to already-completed work.
