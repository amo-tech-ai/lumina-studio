...
> **iPix note (2026-07-08):** This doc is Cloudflare’s guide for **static sites and SPAs** (build dir + `wrangler.jsonc` assets only). **Do not use it for the operator Next.js app.** iPix uses **OpenNext** per the [Next.js on Workers guide](https://developers.cloudflare.com/workers/framework-guides/web-apps/nextjs/). See `cloudflare-vercel.md` and `plan-migrate.md` in this folder.
