> **Supporting / superseded (not domain SSOT).** Active Cloudinary roadmap: [`PLAN.md`](./PLAN.md). Active progress: [`todo.md`](./todo.md).

# Cloudinary Media Operating System — iPix/FashionOS

**Status:** Draft plan, verified against code/schema/account state 2026-07-11. **Corrected 2026-07-11 (same-day follow-up pass):** replaced the `update_access_mode`-based approval design with the documented `access_control` mechanism throughout (§1, §5, §8, §14, §16); made command-response-first (not webhook-first) confirmation explicit; added version-bound approvals and a rights-expiration gate (§8); softened the Mastra/DNA-scoring move from required to optional (§3, §11); corrected the "no migration needed" framing and required a quarantine step before deleting orphaned `cloudinary_assets` rows (§20); added webhook inbox/outbox + dead-letter handling (§14) and a new §21 covering immutable publishing manifests and an original-asset backup/vendor-exit strategy. **Corrected again 2026-07-12 (independent review of the 2026-07-11 pass, verified via `task-verifier` + live doc fetches + the Cloudinary MCP's `asset-update` tool schema):** the 2026-07-11 pass was right to drop `update_access_mode` — confirmed even more strongly, it's explicitly labeled **"Deprecated"** in Cloudinary's own Admin API reference — but was **wrong** to claim no webhook exists for access-control changes; `notification_type: "access_control_changed"` is real and documented. Corrected §1, §5, §8, §14 accordingly: the webhook is now used as a secondary reconciliation signal (not the trigger — API response is still primary and faster). Also added the confirmed webhook retry cadence (3 retries at ~3/6/9 min), a forward-compatible-parsing requirement, `asset_id`-based reconciliation, and a richer orphan-classification vocabulary (§21). Nothing in either pass touched Cloudinary account settings, production data, migrations, or Linear — doc-only, per instruction. **Corrected a third time 2026-07-12 (PR #327 review, verified against live Supabase schema + official Cloudinary docs, not assumed):** (1) the 2026-07-12 pass's own `asset_id` guidance was wrong — `cloudinary_assets.asset_id` is confirmed via live schema to be the **local** `public.assets(id)` foreign key, not Cloudinary's provider asset ID; no column anywhere currently stores Cloudinary's real `asset_id`, so §21 now specifies the actual migration needed instead of a reconciliation shortcut that doesn't exist. (2) `explicit` does not accept `access_control` per Cloudinary's own Upload API reference (confirmed by direct fetch, listing its full parameter set) — every "`explicit`/`update`" reference throughout (§1, §5, §8) is narrowed to the Admin API's per-resource `update` endpoint only. (3) §14's webhook-reliability guidance is expanded to explicitly spell out ack-only-after-inbox-commit, deduplication, a lease/claim processing model, and idempotency for outbound approve/reject calls — the previous wording implied these without stating the actual contract. (4) All "PR #320/#321/#324 unmerged" language throughout the document (§1, §2, §13, §16, §17, §18, Audit, Scoring, Final report) is updated — those three, plus #322 and this file's own two correction PRs (#325, #327), have since merged; §7's `brand_id` resolution gap is updated to reflect PR #329 (merged 2026-07-12), which added campaign-folder resolution and formally tracked the still-unresolved shoot-folder case as its own issue (IPI-524 · SHOOT-ARCH-001) rather than a pending FK question.
...
1. **The rendering/RLS breakage described in the original 2026-07-11 audit is fixed.** The `next.config.ts` fix that lets `next/image` render a signed Cloudinary URL (PR #324), the org-aware RLS policy (PR #321), and the Cloudinary-URL-transform helper (PR #320) all merged on 2026-07-11/12 — confirmed live on `main`. This section is kept as a historical record of what the original audit found broken, not a current-state claim; see the Final report section for present-day status.
...
**Cloudinary account reality check:** iPix is on the **Free plan** (verified via `cloudinary.api.usage()`: 0.05/25 credits used, 480 objects, 261 resources). This matters architecturally: **Creative Approval Proofs/Flows and EasyFlow are Enterprise-only** — not accessible on the current account, no self-serve upgrade path. **PowerFlow is not tier-gated** (metered by "touchpoints" instead) and is the one MediaFlows product actually usable today. Ongoing/automatic moderation requires an Advanced plan or higher; Free caps at 500 total moderation actions. Any plan section proposing Cloudinary-native approval UI must be read against this — the pragmatic path (and the one IPI-437 already independently proposed) is a **self-built approval layer using the `access_control` parameter** (`access_type:"anonymous"|"token"` + `start`/`end`, set via the standard `upload` call or the Admin API's per-resource `update` endpoint — see §8's correction; **not** `explicit`, which does not accept `access_control` at all per Cloudinary's Upload API reference), not Cloudinary's native Enterprise reviewer UI, and **not** the older bulk `update_access_mode` endpoint. **Confirmed precisely (2026-07-12, direct fetch of `image_upload_api_reference` and `admin_api`):** the `access_mode` upload parameter's own docs state "is no longer supported," and the Admin API's `POST /resources/:resource_type/upload/update_access_mode` endpoint is labeled **"Deprecated"** in Cloudinary's own reference table — not merely absent from a subset of pages as this plan's first correction pass (2026-07-11) put it. This is stronger than "legacy/unverified": it is Cloudinary's own explicit deprecation label.
...
### The three fixes that were unmerged at original audit time — merged as of 2026-07-11/12

**Corrected 2026-07-12 (PR #327 review):** all three merged; this section is retained as historical record of the original finding, not a current blocker.

1. `next.config.ts` `images.remotePatterns` missing `/image/authenticated/**` — was: `main` will 500 `next/image` on any real signed asset. **Merged** (PR #324).
2. `assets_select_via_brand` RLS not org-aware — was: non-owner org members see zero assets. **Merged** (PR #321).
3. `withCloudinaryPreset()` (URL-transform injection for legacy/public URLs) — was: existed only on the IPI-404 feature branch. **Merged** (PR #320).

The verification-script split-out (`scripts/verify-rls.mjs` probes) also merged separately as PR #322. This plan's own first correction pass merged as PR #325; this second correction pass is PR #327.
...
cloudinary_assets.approval:"pending" (as today), version pinned at upload time (§8 version-bound rule)
  ...
  → Approve action → server calls the Admin API's per-resource `update` endpoint
    (NOT `explicit` — that API doesn't accept access_control at all, confirmed
    against Cloudinary's own Upload API reference) with
    access_control:[{ access_type:"anonymous" }] (or a token/date-scoped rule for
    rights-expiring assets, §8) → **the synchronous API response is the primary
    confirmation**; write cloudinary_assets.approval:"approved",
    delivery_type:"upload" (or equivalent) directly from that response, in the same
    request/transaction as the approval-event row (§8, §10) — do not *block* on a
    webhook. **Correction (2026-07-12):** a webhook notification for this change
    does exist (`notification_type: "access_control_changed"`, confirmed via direct
    fetch of Cloudinary's `notifications` doc — the plan's first correction pass on
    2026-07-11 was wrong to say none is documented). Use it as a **secondary,
    asynchronous reconciliation signal** — cross-check the webhook payload against
    what the API response already wrote — not as the trigger, since the command
    response arrives first and is authoritative for the write.
...
- **Corrected 2026-07-12 (PR #327 review) — partially resolved since this was written.** `brand_id` resolution originally only worked for brand-folder uploads; PR #329 (merged) added **campaign-folder resolution** too (`campaigns.brand_id` is a real, `NOT NULL` FK — a clean signal, no ambiguity). **Shoot-folder resolution remains unresolved**, but is no longer an open "settle the FK ambiguity" question — PR #329's own investigation found it's a deeper architecture fork: `assets.shoot_id` references `public.shoots` (legacy, no `brand_id` column at all), while the schema that has `brand_id` (`shoot.shoots`) is a separate, currently-unused system the webhook doesn't write to. This is now tracked as its own decision, not a pending lookup — see `IPI-524 · SHOOT-ARCH-001` (not yet resolved as of this pass).
...
3. **Approve** → server action calls the Admin API's **per-resource `update` endpoint** (`POST /resources/:resource_type/:type/:public_id`) with an `access_control` rule (§1, §5) — **not** `explicit`, and **not** the deprecated `update_access_mode` bulk endpoint. **Corrected 2026-07-12 (PR #327 review):** an earlier draft of this section named `explicit` alongside `update` as a valid call site for the `access_control` flip — that was wrong. Confirmed by direct fetch of Cloudinary's Upload API reference: the `explicit` method's full parameter list (`display_name`, `asset_folder`, `eager`, `overwrite`, `tags`, `context`, `metadata`, `moderation`, etc.) does not include `access_control` or `access_mode` at all — only `upload` (initial upload) and the Admin API's resource `update` endpoint accept it. For a post-upload approval flip, that means **`update`, never `explicit`**. Separately confirmed (2026-07-12, prior pass): `update_access_mode` (`POST /resources/:resource_type/upload/update_access_mode`) is labeled **"Deprecated"** in Cloudinary's own Admin API reference, and the `access_mode` upload parameter's docs say it "is no longer supported." `access_control` (`access_type:"token"|"anonymous"` + `start`/`end`) is the current, documented mechanism — this is also what the Cloudinary MCP's own `asset-update` tool schema exposes (`access_control` array with `access_type`/`start`/`end`/`key` — no `access_mode` field at all), independently confirming the same thing at the tool layer. **Before committing to this architecture, run one real, non-destructive `access_control` update against a disposable test asset on the live account and confirm the resulting delivery URL behavior** — this plan has not yet done that live test (§14 open question #1).
4. **Confirmation model:** the synchronous API response from step 3 is the **primary** confirmation — write `cloudinary_assets.approval:"approved"` (and the approval-event row, item 5) from that response directly, in the same request. **Correction (2026-07-12):** a Cloudinary webhook *does* exist for this change (`notification_type: "access_control_changed"`, confirmed via direct fetch of the `notifications` doc — supersedes this plan's 2026-07-11 claim that none is documented). Still don't *block* the write on it — the API response is faster and arrives synchronously — but wire it as a secondary reconciliation signal (§14) that cross-checks the DB state the API response already wrote, catching the rare case where the write succeeded at Cloudinary but the synchronous response was lost (network drop mid-response, server crash before the DB write committed).
...
This is IPI-437/CLD-104's own AC list, refined with three corrections this plan makes beyond the original issue: (a) `access_control`, not the deprecated `update_access_mode`, per the live-doc research above; (b) command-response-first confirmation, using the now-confirmed `access_control_changed` webhook only as secondary reconciliation, not as the trigger; (c) version-bound approval + rights-expiration gate (items 7–8), neither of which the original issue scoped.
...
- `cloudinary_assets.cloudinary_asset_id` (nullable `text`, unique once populated) — Core phase, closes the provider-`asset_id` gap (§21 correction). `cloudinary_assets.asset_id` is the local `public.assets` FK and cannot be repurposed for this.
...
- **The three fixes flagged in §2 (PR #320/#321/#324) have merged** — no longer a standing gap.
...
- RLS on `cloudinary_assets` itself — #321 (org-aware `assets` policy) has merged; whether `cloudinary_assets`'s own RLS mirrors it still hasn't been independently re-checked in any pass so far — ⚪ still an open question, now actionable rather than blocked on a pending merge.
...
**Failure recovery gaps (§7):** no dead-letter queue for failed webhook writes, no alerting, `console.error` only. **Required addition:** treat every inbound Cloudinary webhook call as an event that must be durably recorded before processing — a `cloudinary_webhook_events` **inbox** row written first (raw payload + `notification_type` + `signature_verified` flag + `received_at`), processed, then marked `processed_at`/`last_error`. Failed processing retries from the inbox row (bounded `attempt_count`) instead of relying solely on Cloudinary's own retry behavior — **confirmed exact cadence (2026-07-12, `notifications` doc): Cloudinary retries a non-`200` response 3 additional times at ~3, 6, and 9 minutes after the initial failure (4 attempts total), then stops** — iPix's own retry/dead-letter must not assume Cloudinary will keep trying past that window. A row that exhausts iPix's own retry budget becomes a visible dead-letter entry an operator can inspect — not a silent `console.error`. **Parse defensively:** Cloudinary's own docs warn "Cloudinary may add more fields to API responses and notifications in the future... ensure response parsing remains forward compatible" — the webhook handler must ignore unknown fields rather than fail on them. The same inbox/outbox pattern applies to any outbound Cloudinary API command this plan proposes (§5, §8 approve/reject): log the intent before calling Cloudinary, log the result after, so a crash between the two is detectable and retriable rather than silently lost.

**Corrected/expanded 2026-07-12 (PR #327 review) — the inbox/outbox description above named the pieces but didn't spell out the actual contract. Precisely:**

- **Acknowledge only after the inbox row commits, not before.** The handler's first action is `INSERT INTO cloudinary_webhook_events (...) RETURNING id` inside its own transaction; only once that commit succeeds does the handler return `200` to Cloudinary. If the insert itself fails (DB unreachable, etc.), do **not** return `200` — let Cloudinary's own retry cadence above re-deliver, since nothing was durably recorded yet.
- **Deduplication.** Cloudinary notification payloads don't guarantee a single delivery (that's the entire reason a 3-retry cadence exists) — the inbox insert must be idempotent on a natural key (e.g. `unique(notification_type, public_id, version, received_within_replay_window)`, or a hash of the raw payload) via `ON CONFLICT DO NOTHING`/`DO UPDATE ... WHERE processed_at IS NULL`, so a re-delivered notification doesn't create a second inbox row or re-run side effects that already committed.
- **Lease/claim worker, not "just process it inline."** If processing moves off the request path (recommended once volume grows — a worker polling the inbox table, or a Supabase Edge Function trigger), it must claim a row (`UPDATE ... SET leased_at = now(), leased_by = worker_id WHERE id = $1 AND leased_at IS NULL OR leased_at < now() - lease_ttl`) before processing it, so two workers can't double-process the same event during a slow run. For the current synchronous-in-request-handler design, this reduces to "the handler that inserted the inbox row is the same one that processes it" — the lease model becomes necessary the moment processing is decoupled from the HTTP request.
- **Idempotency for outbound approve/reject calls, not just intent/result logging.** Logging the intent before calling Cloudinary and the result after (as already stated above) records *what happened*, but doesn't by itself prevent a retried approve action from calling Cloudinary's `update` endpoint twice. Because `access_control` updates are naturally idempotent at the Cloudinary end (setting the same rule twice has the same end state), the bigger risk is on the Supabase side: guard the approval write with the same version-bound check already required in §8 item 7 (`WHERE cloudinary_assets.version = $expected_version AND approval = 'pending'`), so a replayed approve request after the first one already succeeded is a no-op, not a duplicate `asset_approval_events` row or an error.
- **Replay protection** is already covered at the transport layer (HMAC signature + 300s window, §7 "Current, working") — the deduplication point above is the *application-layer* complement to that: replay protection stops a stale/forged request from being processed at all; deduplication stops a legitimately-retried, correctly-signed request from being processed twice.

**Open questions — status as of this session's two research passes (2026-07-11 initial, 2026-07-12 follow-up correction):**
1. **Resolved, and this plan's 2026-07-11 pass got it wrong — corrected 2026-07-12:** a direct fetch of Cloudinary's `notifications` doc confirms `access_control_changed` **is** a documented webhook `notification_type` ("updating the access control setting for an asset" is explicitly listed alongside `upload`, `delete`, `moderation`, etc.). The 2026-07-11 pass claimed no such notification exists — that was wrong; it likely came from checking `control_access_to_media` (which doesn't cover webhooks) instead of the `notifications` doc itself. This does **not** change the architecture in §5/§8: the API command's synchronous response should still be the primary/fast confirmation, with the now-confirmed webhook as a secondary reconciliation signal, not a redesign back to webhook-first (a 3–9 minute retry window is too slow to gate an operator-facing "approved" state on).
...
5. Has a real, disposable-asset `access_control` update been tested against the live account yet? Not done as of this pass (only a schema-level check via the Cloudinary MCP's `asset-update` tool was done, confirming the parameter shape, not live behavior) — do this before writing any approve-action code (§1, §8 item 3).
6. MediaFlows/PowerFlow's exact plan-tier availability could not be confirmed from the fetched `mediaflows_user_guide` page (2026-07-12) — it describes touchpoint costs for Premium blocks but not which account tiers can use PowerFlow at all. Verify directly in the Cloudinary Console (Free-plan account) before relying on §9's "usable today" claim.
...
- ~~Merge PR #320, #321, #324~~ — **done**, all merged (§2).
...
- Webhook inbox/outbox with bounded retries + visible dead-letter state, including the ack-after-commit/dedup/lease-claim/idempotency contract (§14) — currently zero durability on a failed write.
- Add `cloudinary_assets.cloudinary_asset_id` (§10, §21) and the one-time backfill job for existing rows.
...
- Approval queue UI with version-bound approval + rights-expiration gate + `access_control` flip (IPI-437/CLD-104, corrected mechanism per §8 — command-response-first, `access_control_changed` webhook as secondary reconciliation only).
...
|---|---|---|---|
| 1 | Merge PR #320, #321, #324 | **Merged** (2026-07-11/12) | Done — was blocking everything else, no longer blocking |
...
| 8 | `IPI-499 — assets_select_via_brand RLS org-aware + shoot.shoot_assets union` | **Done** — org-aware half merged (PR #321, #322); `shoot.shoot_assets` union half re-scoped as `IPI-524 · SHOOT-ARCH-001` (§7 correction) | Part of item 1, org-aware half complete |
...
- [x] PR #320, #321, #324 merged (confirmed 2026-07-12); a real signed Cloudinary upload rendering in `/app/assets` via a live browser session (not just `curl`) is still worth a fresh manual re-check post-merge — not independently re-verified in this pass
...
**Additional corrections from a second review pass (2026-07-12), verified against live Cloudinary docs + the Cloudinary MCP's tool schema:**

- **Corrected 2026-07-12 (PR #327 review — this bullet's earlier claim was wrong, confirmed against live Supabase schema, not assumed):** the previous version of this bullet said "`cloudinary_assets` already captures enough" to key reconciliation off Cloudinary's `asset_id`. **It does not.** Live schema check: `cloudinary_assets.asset_id` is `uuid NOT NULL references public.assets(id)` — the **local** Supabase foreign key to `public.assets`, not Cloudinary's provider asset ID (which Cloudinary documents as a distinct, immutable 32-character hexadecimal string, an entirely different value and format). The webhook (`app/src/app/api/assets/cloudinary/webhook/route.ts`) confirms this at the write site: `upsertCloudinaryAssetRecord` writes `asset_id: assetId`, where `assetId` is the id returned from the `public.assets` upsert — never a value read from the Cloudinary notification payload. **No column in this schema currently stores Cloudinary's real provider `asset_id` at all.**

  Corrected guidance: add a new nullable `text` column — e.g. `cloudinary_assets.cloudinary_asset_id` — via its own additive migration (per §10/§20's migration rules), with a unique index once populated. **New rows going forward:** Cloudinary's `upload`/`eager` notification payloads include the provider's `asset_id` field directly; the webhook should start persisting it into the new column at write time — no reconciliation needed for anything created after this ships. **Existing rows (backfill):** the provider `asset_id` was never captured historically, so it cannot be recovered from data already in Supabase — the only path is a one-time reconciliation job that calls the Cloudinary Admin API's resource-lookup (by `public_id`, still the reliable existing key) for each existing `cloudinary_assets` row, reads back the real `asset_id` from the response, and backfills the new column. Until that backfill runs, §14's reconciliation job should continue keying off `public_id` (already reliable and already stored) — do **not** design the reconciliation job around `asset_id` as the primary key until the new column exists and is populated; treat `public_id`-based matching as the durable default, with `cloudinary_asset_id` as a stronger secondary check once backfilled (since `public_id` is documented as user/rename-changeable, while the provider `asset_id` is immutable).
- **Orphan classification should be richer than binary "orphaned vs. not"** (§20's quarantine correction). A second review pass proposed a useful five-way split — `healthy` / `relink` (Cloudinary resource exists but the Supabase row's `public_id` is stale, e.g. after a rename) / `recover` (Supabase row missing, Cloudinary resource exists) / `quarantine` (ambiguous) / `archive` (Cloudinary resource confirmed gone). This is a better shape than this plan's original two-bucket quarantine-or-orphaned and should be adopted when building the reconciliation job (§14) and CLD-DATA-002's classification step, not just at delete-time.
- **The single `cloudinary_assets.approval` field conflates several independent business decisions** (iPix internal review, client sign-off, DNA/content-safety review, Cloudinary's own moderation result, and the resulting delivery/publish state). This plan's §4/§8 already separates "decision" (Supabase-owned) from "enforcement" (Cloudinary-owned) at a high level, but does not yet split the *decision* side into its own sub-states. A fuller state model (something like separate `ipix_review_status` / `client_approval_status` / `dna_review_status` / `publish_status` fields, with an explicit readiness rule combining all of them) is worth designing properly in `CLD-APPROVAL-001` when that work starts — **not adopted wholesale into this plan now**, since committing to exact column names and enum values ahead of building the approval queue UI (§8, still unbuilt) would be premature detail this plan shouldn't lock in prematurely.
...
|---|---|---|
| **Production correctness** | `main` 500s `next/image` on any real signed asset (§2) | 🟢 Resolved — PR #324 merged |
| **Security** | No RLS org-awareness merged yet on `assets_select_via_brand` (§2, §13) | 🟢 Resolved — PR #321 merged |
...
| **Broken signed-URL flow** | Confirmed and fixed this session (PR #320/#324) | 🟢 Resolved — merged |
| **Provider `asset_id` never captured** | `cloudinary_assets.asset_id` is the local `public.assets` FK, not Cloudinary's provider ID — no column stores the real value (§21 correction, PR #327) | 🟠 Real gap — migration + backfill job needed before any `asset_id`-keyed reconciliation |
| **Webhook dedup/idempotency not fully specified** | Inbox pattern existed but didn't spell out ack-after-commit, dedup key, lease/claim, or outbound-call idempotency (§14 correction, PR #327) | 🟡 Design gap, now specified |
...
| Delivery architecture | 🟡 70 *(was 🔴 55 before #320/#324/#321 merged)* | Rendering/RLS fixed; delivery is still fragmented (two independent code paths, `<CldImage>` unused) |
...
| Production readiness | 🟡 58 *(was 🔴 42 before the 3 PRs merged)* | Rendering/RLS breakage fixed; governance layer (approval/moderation) still schema-only |
| **Overall** | 🟡 **58** *(was 🔴 **42** pre-merge)* | Core pipeline is real and good; governance layer is schema-only |
...
| Area | Result |
|---|---|
| Existing setup score | 🟡 **58/100** on `main` today (2026-07-12) — up from 🔴 42/100 at original 2026-07-11 audit time. **Corrected 2026-07-12 (PR #327 review):** the prior version of this row still described the 3 PRs as pending merge in the rows below, creating a direct contradiction with this row's own stale-flag note. Resolved: PR #320, #321, #322, #324 (and this plan's own #325, #327) have all merged — every row below is updated to match, not left as a historical snapshot |
| Production-ready | Partially — the upload/delivery pipeline (signing, webhook sync, RLS, rendering) is production-ready now that the 3 PRs are merged. Approval/moderation/governance remains schema-only (§2 "code-dark" table) — not production-ready |
| Critical blockers | Rendering (`next/image` 500s) and RLS org-awareness are **resolved** (merged). Still open: (1) webhook silently discards moderation events (§7), (2) upload-sign route's "flip to public" comment has zero implementing code (§2), (3) no column stores Cloudinary's real provider `asset_id` (§21), (4) webhook has no durable inbox/dedup/lease/idempotency contract yet (§14) |
| Highest-value improvement | With rendering/RLS now fixed, the next highest-leverage move is wiring the already-provisioned `cloudinary_assets.moderation_status`/`approval` columns to real events (§7, §8) — the schema work is already paid for and sitting idle |
| Files reviewed | `app/src/lib/cloudinary/url.ts`, `app/src/app/api/_lib/cloudinary-signed-url.ts`, `app/src/app/api/assets/upload-sign/route.ts`, `app/src/app/api/assets/cloudinary/webhook/route.ts` (+test), `app/src/mastra/agents/visual-identity.ts`, `next.config.ts`, `supabase/migrations/*cloudinary_assets*`, `docs/media/*`, `docs/prd/prd-chatwoot.md`, `linear/issues/IPI-257-cloudinary-pipeline.md`, `linear/audit/july-7/07-cloudinary-audit.md`, plus live Supabase (`assets`, `cloudinary_assets`, `shoot.shoot_assets`, `event_assets`) and live Cloudinary Admin API (`usage()`) queries. **2026-07-12 second pass added:** direct fetches of `cloudinary.com/documentation/{notifications, image_upload_api_reference, admin_api, dam_admin_creative_approval_flows, mediaflows_user_guide, cloudinary_moderation}`, and the Cloudinary MCP's `asset-update` tool schema (confirms `access_control`'s exact parameter shape independent of docs). **2026-07-12 third pass (PR #327 review) added:** live `information_schema.columns` query against `public.cloudinary_assets` (confirmed `asset_id` is `uuid NOT NULL` referencing local `public.assets`, not Cloudinary's provider ID); re-read of the current merged `app/src/app/api/assets/cloudinary/webhook/route.ts` to confirm the write site; direct fetch of Cloudinary's `image_upload_api_reference` for the `explicit` method's full parameter list (confirms `access_control` absent); `gh pr view`/`gh api graphql` against PRs #320/#321/#322/#324/#325/#329 for current merge state. |
| Documents created or updated | This file (`tasks/cloudinary/cloudinary-plan.md`, restored from deletion, corrected three times — 2026-07-11, and twice more on 2026-07-12) |
...
| Core phase | ~~Merge 3 PRs~~ (done) → webhook moderation handling; delete-cascade fix; provider-`asset_id` migration + backfill; webhook inbox/dedup/lease/idempotency contract; CLD-000/IPI-430; CI signed-upload smoke test |
...
| Overall recommendation | The 3 PRs that were this plan's Core-phase gate have landed — new Cloudinary feature work is no longer blocked on them. Remaining Core-phase items (webhook moderation handling, delete-cascade, the inbox/dedup/lease/idempotency contract, the provider-`asset_id` migration+backfill) are the next gate before Advanced-phase governance work (approval queue, CLD-104) starts. The 18-issue CLD-101–118 series is a reasonable Advanced/Future roadmap already written; this document's value is sequencing it correctly and flagging where it duplicates older, stale issues. |
