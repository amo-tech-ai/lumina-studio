...
- **Use `asset_id`, not just `public_id`, for provider-side reconciliation.** Confirmed via `admin_api` doc: `asset_id` is "unique and immutable," distinct from `public_id` (which is user-changeable via rename). `cloudinary_assets` already captures enough to add this — when building §14's reconciliation job, key matches off `asset_id` first, falling back to `public_id` only where `asset_id` wasn't captured on older rows.
...
| Area | Result |
|---|---|
| Existing setup score | 🔴 42/100 on `main` today, 🟡 58/100 once PR #320/#321/#324 merge — **stale as of 2026-07-12: PR #320, #321, #322, #324, and this file's own #325 have since merged.** This whole Final-report table is a snapshot from the original 2026-07-11 audit; treat the merge-dependent language below (blockers 1–2, Highest-value improvement) as historical context, not current state — a fresh scoring pass against post-merge `main` is a separate follow-up, not done in this correction pass |
