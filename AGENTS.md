...
Durable, non-obvious notes for agents working in the Cursor Cloud VM. The startup update script already runs `npm ci` (repo root) and `npm ci` in `app/`, so dependencies are present. Scope of the validated dev loop is the **canonical operator app (`app/`)**; the B2C storefront and Mercur marketplace are out of scope for this loop (they need their own Postgres/Redis/Medusa + provider keys).

- **Node:** The VM's default `node` is v22.x. The app pins Node 24 (`app/.nvmrc`, `engines`) to match CI, but v22.x is sufficient here — `npm ci`, `npm run lint`, `npm test`, and `npm run typecheck` all pass on it. Don't waste time forcing Node 24 (nvm has it, but the sandbox PATH prepends its own `node` ahead of nvm).
- **Run the app with `npm run dev:ui` (Next.js on :3002), NOT `npm run dev`.** `npm run dev` also launches the Mastra Studio playground (`mastra dev`, :4111), which **crashes on the committed lockfile**: `mastra@1.1.0-alpha.3`'s bundled `@mastra/deployer` imports `MAX_FS_SUBAGENT_DEPTH` from `@mastra/core@1.41.0`, which no longer exports it (`SyntaxError`). Because `dev` uses `concurrently --kill-others`, that crash also tears down the Next UI. The product does **not** need the :4111 playground — agents run **in-process** through the Next routes `/api/copilotkit` (operator) and `/api/marketing-chat` (public), so `dev:ui` alone is the full app.
- **AI chat needs `GEMINI_API_KEY`** (server-only; default `AI_PROVIDER=gemini`). Without it the app still boots and the chat UI accepts input, but agents return no response. Set it in `app/.env.local` (gitignored) or the run's Secrets. The lightest end-to-end AI check is the **public marketing chat** on `/` (endpoint `/api/marketing-chat`) — it only needs `GEMINI_API_KEY`, no auth.
- **Operator copilot needs a Supabase session too.** `/api/copilotkit` returns `401` without a Supabase access token + org membership even when `OPERATOR_AUTH_ENABLED=false`, and it rejects the dev sentinel ids. Operator dashboard **data** (brands/shoots) needs `NEXT_PUBLIC_SUPABASE_URL` + `NEXT_PUBLIC_SUPABASE_ANON_KEY` against the remote project; without them `/app` renders its shell with "Unable to load dashboard data" / "Portfolio: 0 brands" (expected, not a break).
- **Expected, non-fatal dev-server noise:** an unhandled rejection about `CLOUDFLARE_API_TOKEN` / "Establishing remote connection…" (OpenNext trying to attach remote CF bindings), CopilotKit `/info` returning `401`, and `COPILOTKIT_LICENSE_TOKEN not set` warnings. The server keeps serving on :3002.
- Standard commands (lint/test/typecheck/build) live in `app/package.json` and the **Commands** section above — don't duplicate them.
